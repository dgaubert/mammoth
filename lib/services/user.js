...
function UserService(userDao, userMapper) {
  this.dao = userDao;
    ...
    return this.dao.findByUsername(oldUsername)
      .then(function (user) {
        return oldUsername !== userData.username ?
          [user, _this.dao.checkIfNotExits(userData.username)] :
          [user];
      })
      ...
      .spread(function (user, result /* [salt, hash]*/) {
        ...
        var updatedUser = _.assing(user, data);
...
di.annotate(UserService, new di.Inject(UserDao, UserMapper));

...
module.exports.listPictures = function (slug, callback) {
  var list = [];
  // Establish connection to db
  db.open(function (err, db) {
    if (err) {
      ...
      db.collection('pictures.files').find().toArray(function (err, files) {
        files.forEach(function (file) {
          console.log(file.filename);
          console.log(file.metadata.article);
        });
      });
      db.close();
    }
  ...
  db.open(function (err, db) {
    if (err) {
      ...
      GridStore.list(db, 'pictures', function (err, pictures) {
        if (err) {
          callback(err, null);
        } else {
          pictures.forEach(function (filename) {
            ...
            gridStore.open(function (err, gridStore) {
              if (err) {
                ...
                gridStore.read(function (err, file) {
                  if (err) {
                    callback(err, null);
                  } else {
                    // Close (Flushes the data to MongoDB)
                    console.log(file);
                    if (file.metadata.article === slug) {
                      list.push(filename);
                    }
                    gridStore.close(function (err, result) {
                      if(err) {

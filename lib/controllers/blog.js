var blogController = function blogController(ArticleService) {
  ...
  var controller = {
    ...
    getSummary: function getSummary(req, res, next) {
      ...
      var tasks = {
        articles: function findArticlesPublishedByCategoryOrTagTask(callback) {
          ArticleService.findPublishedByCategoryOrTag(category, tag, page, callback);
        },
        count: function countArticlesPublishedByCategoryOrTagTask(callback) {
    ...
    getArticle: function getArticle(req, res, next) {
      ...
      var tasksDone = function tasksDone(err, blog) {
        ...
        var findByCategoryDone = function findByCategoryDone(err, similars) {
          if (err) {
            return next(err);
          }
    ...
    newComment: function newComment(req, res, next) {
      var slug = req.params.slug ? req.params.slug[0] : null;
      var comment = {

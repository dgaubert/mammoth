...
CommentController.prototype = {
  ...
  create: function (req, res) {
    ...
    if (commentError) {
      return res.status(500).send(commentError);
    }

    this.service.create(slug, comment, captcha)
      .then(function () {
        res.status(200).send('Comentario publicado!. Recargando...');
      })
      .fail(function (err) {
        return res.status(500).send('Comentario no publicado ' + err.message);
      });
  },

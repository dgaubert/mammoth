var articleController = function articleController(ArticleService, PictureService) {
  ...
  var controller = {
    ...
    getArticles: function getArticles(req, res, next) {
      ArticleService.findAll(function findAllDone(err, articles) {
        if (err) {
          return next(err);
        }
    ...
    getArticle: function getArticle(req, res, next) {
      ...
      async.parallel(tasks, function tasksDone(err, result) {
        if (err) {
          return next(err);
        }
        res.render('blog/admin/article', {
